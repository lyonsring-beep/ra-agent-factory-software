...
$('#module-form').onsubmit=async e=>{e.preventDefault();try{show('#lab-result',await call('/modules','POST',formJson(e.target)));loadModules()}catch(x){show('#lab-result',{error:x.message})}};
...
$('#build-form').onsubmit=async e=>{e.preventDefault();const b=formJson(e.target);try{show('#canvas-result',await call(`/compositions/${encodeURIComponent(b.composition_id)}/build`,'POST'))}catch(x){show('#canvas-result',{error:x.message})}};
...
$('#baseline-form').onsubmit=async e=>{e.preventDefault();try{show('#gov-result',await call('/baselines','POST',formJson(e.target)))}catch(x){show('#gov-result',{error:x.message})}};
